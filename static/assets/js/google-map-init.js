var WALNUTSIA_googlemap_init_obj = {};
var WALNUTSIA_GEOCODE_ERROR = "Error";
// Google map Styles
var WALNUTSIA_googlemap_styles = {
    'default': [],
    'style1':  [{"featureType":"administrative","elementType":"labels.text.fill","stylers":[{"color":"#444444"}]},{"featureType":"landscape","elementType":"all","stylers":[{"color":"#f2f2f2"}]},{"featureType":"landscape.man_made","elementType":"geometry.fill","stylers":[{"color":"#eeeeee"}]},{"featureType":"landscape.natural.landcover","elementType":"geometry.fill","stylers":[{"color":"#dddddd"}]},{"featureType":"landscape.natural.terrain","elementType":"geometry.fill","stylers":[{"color":"#dddddd"}]},{"featureType":"poi","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"road","elementType":"all","stylers":[{"saturation":-150},{"lightness":45}]},{"featureType":"road.highway","elementType":"all","stylers":[{"visibility":"simplified"}]},{"featureType":"road.arterial","elementType":"labels.icon","stylers":[{"visibility":"off"}]},{"featureType":"transit","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"water","elementType":"all","stylers":[{"color":"#46bcec"},{"visibility":"on"}]},{"featureType":"water","elementType":"geometry.fill","stylers":[{"color":"#6AB43E"}]},{"featureType":"water","elementType":"labels.text.fill","stylers":[{"color":"#979797"}]},{"featureType":"water","elementType":"labels.text.stroke","stylers":[{"weight":"0.09"}]}],
...
function googlemap_init(dom_obj, coords) {
    ...
    try {
        ...
        var id = dom_obj.id;
        WALNUTSIA_googlemap_init_obj[id] = {};
        WALNUTSIA_googlemap_init_obj[id].dom = dom_obj;
        WALNUTSIA_googlemap_init_obj[id].point = coords.point;
        WALNUTSIA_googlemap_init_obj[id].description = coords.description;
        WALNUTSIA_googlemap_init_obj[id].title = coords.title;
        WALNUTSIA_googlemap_init_obj[id].opt = {
            zoom: coords.zoom,
            center: latlng,
            scrollwheel: false,
            scaleControl: false,
            ...
            styles: WALNUTSIA_googlemap_styles[coords.style ? coords.style : 'default'],
        ...
        custom_map.geocode(coords.latlng!=='' ? {'latLng': latlng} : {"address": coords.address}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                WALNUTSIA_googlemap_init_obj[id].address = results[0].geometry.location;
                googlemap_create(id);
            } else {
                alert(WALNUTSIA_GEOCODE_ERROR + ' ' + status);
            }
        });

        jQuery(window).resize(function() {
            if (WALNUTSIA_googlemap_init_obj[id].map) {
                WALNUTSIA_googlemap_init_obj[id].map.setCenter(WALNUTSIA_googlemap_init_obj[id].address);
            }
        });
...
function googlemap_create(id) {
    "use strict";
    if (!WALNUTSIA_googlemap_init_obj[id].address) { 
        return false;
    }
    WALNUTSIA_googlemap_init_obj[id].map = new google.maps.Map(WALNUTSIA_googlemap_init_obj[id].dom, WALNUTSIA_googlemap_init_obj[id].opt);
    WALNUTSIA_googlemap_init_obj[id].map.setCenter(WALNUTSIA_googlemap_init_obj[id].address);
    var markerInit = {
        map: WALNUTSIA_googlemap_init_obj[id].map,
        position: WALNUTSIA_googlemap_init_obj[id].address   //WALNUTSIA_googlemap_init_obj[id].map.getCenter()
    };
    if (WALNUTSIA_googlemap_init_obj[id].point) { 
        markerInit.icon = WALNUTSIA_googlemap_init_obj[id].point;
    }
    if (WALNUTSIA_googlemap_init_obj[id].title) { 
        markerInit.title = WALNUTSIA_googlemap_init_obj[id].title;
    }
    var marker = new google.maps.Marker(markerInit);
    var infowindow = new google.maps.InfoWindow({
        content: WALNUTSIA_googlemap_init_obj[id].description
    });
    google.maps.event.addListener(marker, "click", function() {
        infowindow.open(WALNUTSIA_googlemap_init_obj[id].map, marker);
    });
}


function googlemap_refresh(){
    for(id in WALNUTSIA_googlemap_init_obj){
        googlemap_create(id);
    }
}
