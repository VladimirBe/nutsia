...
def handler400(request):
    ...
    context={'title': '400 Bad Request', 'message': '400'}
    response = render(request, 'error_page.html', context)
    response.status_code = 400
    return response


def handler403(request):
    context={'title': '403 Permission Denied', 'message': '403'}
    response = render(request, 'error_page.html', context)
...
def handler404(request):
    context={'title': '404 Not Found', 'message': '404'}
    response = render(request, 'error_page.html', context)
...
def handler500(request):
    context={'title': '500 Server Error', 'message': '500'}
    response = render(request, 'error_page.html', context)
...
def galery(request):
    path="/home/Walnutsia/walnutsia/static/assets/images/galeria"
...
class ContactView(generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if context['form'].is_valid():
            ...
            ctx = {
                'success': """Vă mulțumim pentru că ne-ați scris, promitem că revenim cu un răspuns"""
            }
